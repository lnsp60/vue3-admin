<template>
  <el-card>
    <el-select style="width: 300px;" v-model="value" placeholder="选择站点名称" filterable>
      <el-option 
  ...
  <el-card class="mt">
    <el-row :gutter="20">
      <el-col :span="8" v-for="item in dataListCopy" :key="item.id" class="mb">
        <div class="item">
          <div class="pic">
        ...
        <div class="btn">
            ...
            <div>
              ...
              <div class="fl ml" style="text-align: right;">
                ...
                <el-popover placement="right" :width="300" trigger="click">
                  ...
                  <el-timeline style="max-width: 300px;">
                    <el-timeline-item v-for="i in item.record" :key="i.time" :timestamp="i.time" hollow>
                      {{ i.msg }}
                    </el-timeline-item>
                  </el-timeline>
                </el-popover>
...
const option = ref<any>([])//下拉菜单数据
const dataList = ref<any>([])//列表数据
const dataListCopy = ref<any>([])//列表数据备份

const loadData = async () => {
  const { data } = await currentListApi()
  option.value = data
  dataList.value = data[0].list//原始数据
  dataListCopy.value = data[0].list//列表渲染
}
...
watch(value,() => {
  const res = option.value.filter((item:any) => item.name===value.value)
